...
class WipImport implements ToModel, WithHeadingRow, WithStartRow
{
    public $errorRows = [];
    ...
    public function model(array $row)
    {
        ...
        $existingWip = Wip::where('inventory_id', $row['inventory_id'])
        ...
        try {
            if ($existingWip) {
                ...
                $this->successfulRows[] = $existingWip;
            } else {
                ...
                $this->successfulRows[] = $wip;
            }

            DB::commit(); // Commit jika berhasil
            return $existingWip ?? $wip;
        } catch (\Exception $e) {
