...
@section('content')
   <div class="pagetitle">
       ...
       <nav>
           <ol class="breadcrumb">
               ...
               <li class="breadcrumb-item active"> Data Cipat</li>
           </ol>
       </nav>
   </div>
   ...
   <section class="section">
       <div class="card">
           <div class="card-body">
               <h5 class="card-title">Cipat List</h5>

               <a href="{{ route('cipat.create') }}" class="btn btn-primary mb-3">
                       ...
                       <tbody>
                           @foreach($cipat as $index => $item)
                               <tr>
                                   ...
                                   <td>{{ $item->satuan }}</td>
                                   <td>{{ $item->stok_awal }}</td> <!-- New column data -->
                                   <td>{{ $item->plant }}</td>
                                   <td>
                                       <div class="d-flex justify-content-center">
                                           <a href="{{ route('cipat.edit', $item->id) }}" class="btn btn-primary me-2">
                                               <i class="fas fa-edit"></i> Edit
                                           </a>
                                           <form action="{{ route('cipat.destroy', $item->id) }}" method="POST" id="delete-form-{{ $item->id }}" style="display:inline;">
                                               @csrf
                                               @method('DELETE')
       ...
       <div class="modal-dialog">
           <div class="modal-content">
               <div class="modal-header">
                   <h5 class="modal-title" id="importModalLabel">Import Cipat from Excel</h5>
                   <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
               </div>
               <div class="modal-body">
                   <form id="importForm" action="{{ route('cipat.import') }}" method="POST" enctype="multipart/form-data">
                       @csrf
                       <div class="mb-3">
       ...
       function changeStatus(partId, status) {
           Swal.fire({
               ...
               if (result.isConfirmed) {
                   fetch(`/cipat/${partId}/change-status`, {
                       method: 'POST',
                       headers: {
